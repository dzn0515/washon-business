...
export default function AdminSalesDistributorsPage() {
  ...
  const [account, setAccount] = useState<AdminSalesDistributorAccount | null>(null)
  ...
  const handleIssueAccount = async () => {
    ...
    setAccountError(null)
    try {
      const res = await createAdminSalesDistributorAccount(editing.id, {
  ...
  const handleAccountStatus = async () => {
    ...
    setAccountError(null)
    try {
      const res = await updateAdminSalesDistributorAccountStatus(
        editing.id,
  ...
  const handleResetPassword = async () => {
    ...
    setAccountError(null)
    try {
      await resetAdminSalesDistributorAccountPassword(editing.id, resetPassword)
      setResetConfirmOpen(false)
      setResetPassword('')
      setShowResetPassword(false)
      ...
      >
        <p className="text-sm text-gray-600">
          {statusConfirm === 'deactivate'
            ? '포털 로그인을 비활성화할까요? 총판 정보와 하위 조직은 유지됩니다.'
            : '포털 로그인을 다시 활성화할까요?'}
        </p>
      </AdminModal>
