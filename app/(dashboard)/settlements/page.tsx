...
export default function SettlementsPage() {
  const { summary, daily, year, month } = mockSettlements
  const pending = daily
    .filter((r) => r.status === 'PENDING')
    .reduce((s, r) => s + r.settled_amount, 0)

  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <select className="px-3 py-2 border border-gray-200 rounded-xl text-sm" defaultValue={`${year}-${month}`}>
          <option value="2026-06">{year}년 {month}월</option>
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card><div className="text-xs text-gray-400">총 매출</div><div className="text-base font-semibold">{formatMoney(summary.total_sales)}</div></Card>
        <Card><div className="text-xs text-gray-400">플랫폼 수수료</div><div className="text-base font-semibold">{formatMoney(summary.platform_fee)}</div></Card>
