...
import {
  fetchDistributorSettlements,
  type DistributorSettlementListItem,
...
export default function DistributorSettlementsPage() {
  const [items, setItems] = useState<DistributorSettlementListItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const list = await fetchDistributorSettlements({ page, pageSize: PAGE_SIZE })
      setItems(list.items)
      setTotal(list.total)
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-stone-500">총판 몫 확정 정산 내역입니다.</p>
        </div>
              ...
              items.map((item) => (
                <tr key={item.id} className="hover:bg-stone-50/80">
                  ...
                  <td className="px-4 py-3">
                    <Link
                      href={`/distributor/settlements/${item.id}`}
