...
import { fetchDistributorLeads, type DistributorLead } from '@/lib/distributor-api'
...
export default function DistributorLeadsPage() {
  const [items, setItems] = useState<DistributorLead[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetchDistributorLeads({
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-stone-500">조직 소속 영업사원이 작성한 신청을 조회합니다. (승인 권한 없음)</p>
      </div>
              ...
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  ...
                  <p className="mt-1 text-xs text-stone-400">
                    사원 {row.salesAgentName ?? '-'} · 영업점 {row.agencyName ?? '-'}
                  </p>
                </div>
