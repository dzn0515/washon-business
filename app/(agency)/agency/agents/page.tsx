...
import { fetchAgencyAgents, type AgencyAgentListItem } from '@/lib/agency-api'
...
export default function AgencyAgentsPage() {
  const [items, setItems] = useState<AgencyAgentListItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetchAgencyAgents({
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-stone-500">소속 영업사원을 조회합니다.</p>
      </div>
      <form onSubmit={(e) => { e.preventDefault(); setPage(1); setKeyword(keywordInput.trim()) }} className="flex flex-wrap gap-2">
        <input value={keywordInput} onChange={(e) => setKeywordInput(e.target.value)} placeholder="이름·코드 검색" className="min-w-[180px] flex-1 rounded-lg border border-stone-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
      ...
      <div className="overflow-x-auto rounded-xl border border-stone-200 bg-white">
        <table className="w-full min-w-[560px] text-left text-sm">
          ...
          <tbody className="divide-y divide-stone-100">
            {loading ? (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-stone-400">불러오는 중...</td></tr>
            ) : items.length === 0 ? (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-stone-400">영업사원이 없습니다.</td></tr>
            ) : (
              items.map((item) => (
                <tr key={item.id} className="hover:bg-stone-50/80">
                  <td className="px-4 py-3">
                    <Link href={`/agency/agents/${item.id}`} className="font-medium text-emerald-800 hover:text-emerald-700">{item.name}</Link>
